...
public class HitBox extends Check {
    ...
    public void onPacket(Object packet, String packetType, long timeStamp) {
        if(packetType.equalsIgnoreCase(Packet.Client.USE_ENTITY)) {
            ...
            if(target instanceof Player) {
                val entityData = Iris.getInstance().getDataManager().getPlayerData(target.getUniqueId());
    ...
    private BoundingBox getHitbox(LivingEntity entity, CustomLocation l) {
        ...
        return new BoundingBox(0, 0, 0, 0, 0, 0).add((float) l.getX(), (float) l.getY(), (float) l.getZ()).grow((float) dimensions.getX(), (float) dimensions.getY(), (float) dimensions.getZ()).grow(.25f, 0.25f, .25f)
